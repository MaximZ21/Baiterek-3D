...
public class ThickCircle {
    ...
    public ThickCircle() {
        ...
        for (double theta = 0.0; theta <= (Math.PI * 2); theta += dTheta) {
            ...
            points++;
            circleVertex.put((float) (mRaduis * Math.cos(theta)));//1
            circleVertex.put(ShiftY+widthY);//3
            circleVertex.put((float) (mRaduis * Math.sin(theta)));//2
            points++;
            circleVertex.put((float) (mRaduis * Math.cos(theta+dTheta)));//1
            circleVertex.put(ShiftY);//3
...
//        }
        int var = 37;
        vertexCount = points;
        circleVertex.position(0);
        triangleCoords = circleVertex.array();
